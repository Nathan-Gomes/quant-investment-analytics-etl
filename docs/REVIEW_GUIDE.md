...
It turns raw price and holdings data into validated portfolio accounting, historical return and drawdown comparisons, and equal-capital five-year scenario ranges. It doesn't claim to know which portfolio will win, or predict any specific future price.

## The short path through it
...
6. Open `output/report.html` or `output/notebook.html` to see the results without reading any code.
...
Selection and survivorship risk are still present: the universe and allocation rules were chosen retrospectively, and adjusted-price data isn't a point-in-time archive.
...
- Balanced had the strongest observed risk-adjusted result, which would matter more under a drawdown budget.
...
- The scenario outputs help compare risk tolerances and raise questions worth digging into. They don't pick a best allocation.
