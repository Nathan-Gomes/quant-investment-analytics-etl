...
nb.cells[4:4] = [
    ...
    nbf.v4.new_code_cell("from IPython.display import Image\nfor filename in ['forward_indexed.png', 'scenario_downside.png', 'scenario_drawdowns.png', 'forward_ranges.png']:\n    display(Image(filename=str(ROOT / 'output' / filename)))\nfor table in ['forward_projection_summary', 'scenario_downside', 'scenario_paired', 'scenario_sensitivity']:\n    display(pd.read_sql(f'SELECT * FROM {table}', conn))\ndisplay(Image(filename=str(ROOT / 'output' / 'forward_scenarios.png')))"),
    nbf.v4.new_markdown_cell("## Downside interpretation\nThe 5th percentile is a cutoff, not the mean of adverse outcomes. The worst-5% mean averages terminal wealth below that cutoff. Paired comparisons subtract Balanced from Growth inside each identical scenario; differences of separate percentiles do not give percentiles of that difference. Maximum drawdown records losses from prior peaks even when terminal wealth recovers. Longer moving blocks change dependence and underweight sample endpoints; circular blocks reduce endpoint weighting but create artificial end-to-start joins. These sensitivity runs are not independently validated forecasts and are not selected to produce a preferred result."),
...
for name in ("extract", "validation", "analytics", "forward", "scenario_risk", "risk_presentation", "models", "load", "pipeline"):
